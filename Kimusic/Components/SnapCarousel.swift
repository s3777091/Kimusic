...
//Struct for converting PurpleItemPodCast into an identifiable object for SnapCarousel
struct forSnap: Identifiable{
    var item: PurpleItemPodCast
    var id = UUID()
    //convert PurpleItemPodCast array into forSnap array
    static func getArr(items: [PurpleItemPodCast]) -> [forSnap]{
        var newForSnap: [forSnap] = []
        if (items.endIndex - 1 > 0){
            for i in 0 ..< (items.endIndex - 1){
                newForSnap.append(forSnap.init(item: items[i]))
            }
        }
        else {
            print(items.endIndex)
        }
        return newForSnap
    }
}
...
struct SnapCarousel<Content: View>: View {
    ...
    init(spacing: CGFloat = 15,trailingSpace: CGFloat = 200,index: Binding<Int>,items: [forSnap], @ViewBuilder content: @escaping (forSnap)->Content){
    ...
    func getOffset(item: forSnap,width: CGFloat)->CGFloat{
    ...
    // Fetching Index...
    func getIndex(item: forSnap)->Int{
